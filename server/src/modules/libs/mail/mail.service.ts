...
export class MailService {
	...
	async sendChannelVerificationEmail(email: string) {
		const html = await render(VerifyChannelTemplate())
		return this.sendMail(email, 'Channel Verification', html)
	}

	async sendEnableTwoFactorEmail(email: string, token: string) {
		// const domain = this.configService.getOrThrow<string>('ALLOWED_ORIGIN')
		const domain = this.configService.getOrThrow<string>('MAIL_DOMAIN')
